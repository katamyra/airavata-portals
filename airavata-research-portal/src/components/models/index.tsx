...
export const Models = () => {
  ...
  const fetchModels = async () => {
    try {
      setLoading(true);
      setError(null);
      const data = await adminApiService.getModels();
      setModels(data);
    } catch (err) {
      console.error("Failed to fetch models:", err);
      setError("Failed to load models. Make sure the API server is running.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleStar = async (id: number) => {
    try {
      const updatedModel = await adminApiService.starModel(id);
      setModels(prev => 
        prev.map(model => 
          model.id === id ? { ...model, starCount: updatedModel.starCount } : model
        )
      );
    } catch (err) {
      console.error("Failed to star model:", err);
    }
  };

  const filteredModels = activeFilter === "All" 
    ? models 
    : models.filter(model => model.category === activeFilter.toLowerCase().replace(" ", "_"));
        ...
        <VStack spacing={10} align="stretch">
          ...
          <Flex justify="space-between" align="start">
            <Box flex={1}>
              ...
              <Text color="gray.600" maxW="600px" lineHeight="1.6" fontSize="md">
                Access and manage machine learning models effortlessly in CyberShuttle. Run them 
                in notebooks or register your own models to integrate with compute workflows.
              </Text>
            </Box>
          ...
          <Box>
            <Button
              ...
              _hover={{ bg: "#4a9ce6" }}
              borderRadius="md"
              onClick={() => navigate('/resources/models/new')}
            >
              New Models
            </Button>
          </Box>
          ...
          {error && (
            <Box textAlign="center" py={12}>
              ...
              <Button mt={4} onClick={fetchModels} size="sm">
                Retry
              </Button>
            </Box>
          )}
          ...
          {!loading && !error && filteredModels.length === 0 && (
            <Box textAlign="center" py={12}>
              <Text color="gray.500" fontSize="lg">
                No models found for "{activeFilter}"
              </Text>
